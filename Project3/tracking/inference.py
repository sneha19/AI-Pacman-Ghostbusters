        ...
        if noisyDistance == None:
            ...
            for loc in self.legalPositions:
                ...
                if emissionModel[dist] > 0:
                    allPossible[loc] = emissionModel[dist] * self.beliefs[loc]
                else:
                    continue
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            dist = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            Items = dist.items()
            for loc, c in Items:
                allPossible[loc] = allPossible[loc] + (c * self.beliefs[pos])
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        c = 0
        partList = self.particles = []
        nop = self.numParticles
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: 
            ...
            allPossible = util.Counter()
            #val = allPossible.values()
            ob = self.getBeliefDistribution()
            for loc in legal:
                dist = util.manhattanDistance(loc, pacmanPosition)
                allPossible[loc] = allPossible[loc] + (emissionModel[dist] * ob[loc])
            ...
            if not any(allPossible.values()):
                ...
                for _ in range(0, self.numParticles):
                    #making a sample 
                    toReturn.append(util.sample(allPossible)) 

                self.particles = toReturn
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        partList = []
        #oldPartList = self.particles
        for pos in self.particles:
            dist = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            newDist = util.sample(dist)
            partList.append(newDist)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        random.shuffle(pos) #shuffling list of permutations
        c = 0
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for c, p in enumerate(self.particles):
            temp = 1.0
            for i in range(r):
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                gpos = setGhostPositions(gameState, newParticle)
                dist = getPositionDistributionForGhost(gpos, i, self.ghostAgents[i])
                #getting sample distance
    ...
    def getBeliefDistribution(self):
        ...
        for i in self.particles:
            distribution[i] = distribution[i] + 1.0
